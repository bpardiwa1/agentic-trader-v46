...
def _is_session_open(env, now: dt.datetime, symbol: str) -> bool:
    ...
    try:
        ...
        return True  # fallback: allow trade if misconfigured
...
def compute_features(symbol: str, env=ENV) -> Dict[str, Any]:
    """Compute EMA, RSI, ATR, and session flags for indices."""
    tf = env.get("TIMEFRAME", "M15") or "M15"
    ...
    # Log in unified style with XAU/FX
    log.info(
        "[DEBUG] %s TF=%s EMA_FAST=%.2f EMA_SLOW=%.2f GAP=%.2f RSI=%.2f ATR%%=%.4f | EPS=%.2f | SL=%.1f TP=%.1f",
        symbol, tf, efast, eslow, ema_gap, rsi_val, atr_pct, eps, sl_dyn, tp_dyn
    )
