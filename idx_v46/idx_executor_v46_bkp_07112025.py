# ============================================================
# Agentic Trader IDX v4.6 — Execution Engine (CM07)
# ------------------------------------------------------------
# Enhancements:
# • Fix KeyError 'sid' in order_send()
# • Add symbol-aware & price-aware broker-safe SL/TP recalibration
# • Retain trust/confidence/dynamic lot scaling
# • CM07: Prevent retcode=10016 (Invalid stops)
# ============================================================
...
def execute_trade(symbol: str, decision: Dict[str, Any], feats: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # --- Symbol-aware widening base (CM06 logic)
    ...
    limit_total = int(ENV.get("AGENT_MAX_OPEN", 12))
    limit_symbol = int(ENV.get("AGENT_MAX_PER_SYMBOL", 1))
    ...
    if result.get("ok"):
        log.info("[EXECUTED] %s %s lots=%.2f SL=%.1f TP=%.1f conf=%.2f",
                 symbol, side, lots, sl_scaled, tp_scaled, final_conf)
    else:
