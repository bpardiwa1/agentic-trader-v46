# idx_v46/watchers/idx_telegram_watcher.py
# ============================================================
# Agentic Trader IDX v4.6 — Telegram Watcher (LOG or JSONL)
# ------------------------------------------------------------
# WATCH_MODE:
...
# Requirements:
#   - No external deps
#   - Loads env vars from: idx_v46/watchers/idx_watcher.env
# ============================================================
...
def load_cfg() -> Cfg:
    return Cfg(
        ...
        # LOG mode
        log_dir=_env("WATCH_LOG_DIR", r"logs\idx_v4.6"),
        log_glob=_env("WATCH_LOG_GLOB", "idx_v46_*.log"),
        event_prefix=_env("WATCH_EVENT_PREFIX", "EVENT").strip(),

        # JSONL mode
        jsonl_file=_env("WATCH_JSONL_FILE", ""),
        jsonl_glob=_env("WATCH_JSONL_GLOB", ""),
        state_file=_env("WATCH_STATE_FILE", r"logs\idx_v4.6\idx_watcher.state.json"),
...
def _send_attach(cfg: Cfg, mode: str, file_path: str, pos: int, extra: str = "") -> None:
    """
    Send/print an attach message including the actual file and byte offset.
    """
    base = f"[IDX][WATCHER] attached (mode={mode})"
    fp = _short_path(file_path) if file_path else "-"
    msg = f"{base}\nfile: {fp}\npos: {int(pos)}"
    if extra:
        msg += f"\n{extra}"

    # Console
    full_fp = file_path or "-"
    print(f"[WATCHER][ATTACH] mode={mode} file={full_fp} pos={int(pos)}")

    # Telegram
    tg_safe_send(cfg, msg, tag="ATTACH")
...
def _watch_log(cfg: Cfg) -> None:
    ...
    # Initial attach (will update once we pick the newest file)
    if cfg.attach_message:
        _send_attach(cfg, "log", current_file, file_pos, extra=f"dir: {cfg.log_dir}\nglob: {cfg.log_glob}")
    ...
    while True:
        try:
            ...
            if newest and newest != current_file:
                ...
                if cfg.attach_message:
                    _send_attach(cfg, "log", current_file, file_pos)
            ...
            if cfg.heartbeat_enabled:
                ...
                if now - last_heartbeat >= cfg.heartbeat_interval_sec:
                    parts = ["[IDX][HEARTBEAT] watcher alive (mode=log)"]
                ...
                if cfg.heartbeat_silence_alert_sec > 0 and last_event_ts > 0:
                    ...
                    if silence >= cfg.heartbeat_silence_alert_sec:
                        parts = ["[IDX][SILENCE] no events seen recently"]
                        if cfg.heartbeat_include_file:
                            parts.append(f"file: {_short_path(current_file) if current_file else '-'}")
...
def _watch_jsonl(cfg: Cfg) -> None:
    ...
    # Attach message with current state (may be empty until we find target)
    if cfg.attach_message:
        extra = f"state: {_short_path(cfg.state_file)}"
        _send_attach(cfg, "jsonl", current_file, file_pos, extra=extra)
    ...
    while True:
        try:
            ...
            if target and target != current_file:
                ...
                if cfg.attach_message:
                    _send_attach(cfg, "jsonl", current_file, file_pos, extra=f"state: {_short_path(cfg.state_file)}")
            ...
            if cfg.heartbeat_enabled:
                now = _now_ts()
                if now - last_heartbeat >= cfg.heartbeat_interval_sec:
                    parts = ["[IDX][HEARTBEAT] watcher alive (mode=jsonl)"]
                    if cfg.heartbeat_include_file:
                        parts.append(f"file: {_short_path(current_file) if current_file else '-'}")
                    if cfg.heartbeat_include_last_event and last_event_ts > 0:
                        ago = int(now - last_event_ts)
...
def main() -> None:
    ...
    env_path = os.path.join(here, "idx_watcher.env")
