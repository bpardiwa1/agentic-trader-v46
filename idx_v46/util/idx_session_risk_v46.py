# idx_v46/util/idx_session_risk_v46.py
# ------------------------------------------------------------
# Session Risk Controller (SRC) for IDX v4.6
...
_LOG_DIR = "logs/idx_v4.6"
_LOG_LEVEL = str(ENV.get("IDX_LOG_LEVEL", ENV.get("LOG_LEVEL", "INFO"))).upper()
log = setup_logger("idx_session_risk_v46", log_dir=_LOG_DIR, level=_LOG_LEVEL)
...
def _pnl_and_streak(symbol: str, deals_today: list) -> tuple[float, float, int]:
    """Compute account PnL, symbol PnL, and symbol consecutive-loss streak for today."""
    total_pnl = 0.0
    sym_pnl = 0.0
    sym_deals = []
    ...
    # Sort symbol deals by time ascending; walk backwards for streak
    ...
    for d in reversed(sym_deals):
        ...
        if profit < 0:
            ...
            # zero-profit closes break the streak
            break

    return total_pnl, sym_pnl, consec_losses


def check_idx_risk(symbol: str) -> Dict[str, Any]:
    """
    Check session-level risk limits for an index symbol.
    ...
    # Note: thresholds are positive numbers, representing max allowed loss in account currency.
    # --- Env thresholds (all optional, with per-symbol overrides) --
    # Global account-wide DD stop
    max_dd_acc = float(ENV.get("IDX_SRC_MAX_DD_DAY_ACC", 0.0))  # e.g. 400.0

    # Per-symbol overrides based on base name (NAS100, HK50, UK100, …)
    base = symbol.split(".", 1)[0].upper()  # "NAS100.s" -> "NAS100"

    max_dd_sym = float(
        ENV.get(
            f"IDX_SRC_MAX_DD_DAY_SYM_{base}",
            ENV.get("IDX_SRC_MAX_DD_DAY_SYM", 0.0),
        )
    )
    max_consec = int(
        ENV.get(
            f"IDX_SRC_MAX_CONSEC_LOSSES_{base}",
            ENV.get("IDX_SRC_MAX_CONSEC_LOSSES", 0),
        )
    )
    cooloff_min = int(
        ENV.get(
            f"IDX_SRC_COOL_OFF_MIN_{base}",
            ENV.get("IDX_SRC_COOL_OFF_MIN", 60),
        )
    )
