# idx_v46/util/idx_event_sink_v46.py
# ============================================================
# IDX v4.6 — Shared JSONL Event Sink (Watcher-friendly)
# ------------------------------------------------------------
# Writes:
#   1) log line:  EVENT {...}   (optional)
#   2) jsonl line: {...}        (always, best-effort)
#
# Daily rotation:
#   logs/idx_v4.6/idx_events_YYYY-MM-DD.jsonl   (default)
# ============================================================
...
def _events_dir() -> str:
    return str(ENV.get("IDX_EVENTS_DIR", "logs/idx_v4.6"))
...
def _events_path_for_today() -> str:
    ...
    if _daily_rotate_enabled():
        fname = f"idx_events_{datetime.now():%Y-%m-%d}.jsonl"
    else:
        fname = "idx_events.jsonl"
