...
# ------------------------------------------------------------
# Session tagging (for dashboards / attribution)
# ------------------------------------------------------------


def _fx_session_tag(ts: datetime) -> str:
 """Coarse FX session tag in Malaysia local time (no TZ conversion).
 Used only for analytics/attribution (not trading logic).
 """
 try:
     t = ts.time()
     # Rollover/illiquid window (~05:00–07:00 MYT)
     if _dtime(5, 0) <= t < _dtime(7, 0):
         return "ROLLOVER"
     # Asia
     if _dtime(7, 0) <= t < _dtime(13, 0):
         return "ASIA"
     # London
     if _dtime(13, 0) <= t < _dtime(16, 0):
         return "LONDON"
     # London/NY overlap
     if _dtime(16, 0) <= t < _dtime(20, 0):
         return "LDN_NY"
     # NY late
     if _dtime(20, 0) <= t <= _dtime(23, 59, 59):
         return "NY"
     # Default catch-all
     return "ASIA"
 except Exception:
...
class FxAgentV46:
 ...
 def _run_symbol(self, sym: str, summary: dict):
     ...
     try:
         ...
         decision_reason = preview.get("decision_reason") or preview.get("reason")
         session = _fx_session_tag(datetime.now())
         ...
         _emit_event(
             "DECISION",
             module="agent",  # PATCH: was "decider" (agent should not impersonate decider)
             cycle=cid,
             symbol=sym,
             side=side or "",
             confidence=conf,
         ...
         # -----------------------------------------------------------
         # 4b) Decision-context kill-switch (must have canonical reason)
         # -----------------------------------------------------------
         if not decision_reason:
             log.warning("[BLOCKED] %s %s missing decision_reason; blocking execution", sym, side)
             _emit_event(
                 "BLOCKED",
                 module="agent",
                 cycle=cid,
