...
def _primary_reason(side: str, why: list[str]) -> str:
    """Pick a single canonical decision reason for analytics / dashboards."""
    if not why:
        return "no_reason"

    # Prefer explicit entry triggers
    for k in ("ema_rsi_bull", "ema_rsi_bear"):
        if k in why:
            return k

    # Trend-only tag (if we traded, tag it; if blocked, we'll be SKIP anyway)
    if "trend_only" in why:
        return "trend_only"

    # Volatility tags (useful for clustering)
    for k in ("atr_quiet", "atr_hot"):
        if k in why:
            return k

    # Policy blocks (mostly for skips)
    for k in ("policy_strict_block", "policy_flexible_block", "policy_aggressive_trend_miss"):
        if k in why:
            return k
...
def decide_signal(feats: dict, env) -> dict:
    ...
    preview = {
        ...
        "trend_h1": trend_h1,
        "why": why,
        "decision_reason": _primary_reason(side, why),
    }
    ...
    _emit_event(
        ...
        why=why,
        decision_reason=str(_primary_reason(side, why)),
    ...
    return {"accepted": accepted, "preview": preview, "decision_reason": preview.get("decision_reason", "no_reason")}
