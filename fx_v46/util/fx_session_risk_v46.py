# fx_v46/util/fx_session_risk_v46.py
# ------------------------------------------------------------
# Session Risk Controller (SRC) for FX v4.6
...
_FX_LOG_LEVEL = str(ENV.get("FX_LOG_LEVEL", ENV.get("LOG_LEVEL", "INFO"))).upper()
log = setup_logger("fx_session_risk_v46", log_dir=_FX_LOG_DIR, level=_FX_LOG_LEVEL)
...
def check_fx_risk(symbol: str) -> Dict[str, Any]:
    """
    Check session-level risk limits for an FX symbol.
    ...
    max_dd_acc = float(ENV.get("FX_SRC_MAX_DD_DAY_ACC", 0.0))   # e.g. 300.0
    max_dd_sym = float(ENV.get("FX_SRC_MAX_DD_DAY_SYM", 0.0))   # e.g. 150.0
    max_consec = int(ENV.get("FX_SRC_MAX_CONSEC_LOSSES", 0))    # e.g. 3
    cooloff_min = int(ENV.get("FX_SRC_COOL_OFF_MIN", 60))       # e.g. 60
