...
KIND_RE = re.compile(
    r"\[(?P<kind>DECIDE|PREVIEW|EXECUTOR|AGENT)\]\s+(?P<sym>[A-Za-z0-9_.\-]+)"
)
...
ATR_REGIME_RE = re.compile(r"\batr_regime=(?P<atr_regime>[A-Za-z0-9_\-]+)")
SESSION_RE = re.compile(r"\bsession=(?P<session>[A-Za-z0-9_\-]+)")
WHY_KV_RE = re.compile(r"\bwhy=(?P<why>[^)\n\r]+)")
...
def parse_exec_events(tag: str) -> List[ExecEvent]:
    ...
    XAU compatibility:
      - XAU agent may emit execution confirmation as: [AGENT] <sym> <side> ok (... policy=..., atr_regime=..., session=..., why=a|b|c)
      - Some XAU logs may not include [EXECUTOR] lines. We treat [AGENT] ok lines as ExecEvents.
      - We do NOT require KIND_RE to include AGENT; we fallback-parse AGENT lines safely.
    """
    ...
    # Local regex helpers (kept local to avoid breaking other modules)
    AGENT_RE = re.compile(r"\[AGENT\]\s+(?P<sym>[A-Za-z0-9_.\-]+)\b")
    ATR_REGIME_KV_RE = re.compile(r"\batr_regime=(?P<atr>[A-Za-z0-9_\-]+)")
    WHY_KV_RE = re.compile(r"\bwhy=(?P<why>[^)\n\r]+)")
    # (Optional) If you later want session, you can parse it similarly:
    # SESSION_KV_RE = re.compile(r"\bsession=(?P<session>[A-Za-z0-9_\-]+)")

    def _parse_preview_reasons_from_line(line: str) -> List[str]:
        """
        PREVIEW lines sometimes use reason=[...] while DECIDE uses why=[...].
        Your old code used REASON_RE but then mw.group("why") which can fail silently.
        This keeps it robust without changing your global regexes.
        """
        mw = REASON_RE.search(line)
        if not mw:
            return []
        gd = mw.groupdict()
        raw = gd.get("why") or gd.get("reason")
        if raw is None:
            # Fallback: first capturing group if present
            try:
                raw = mw.group(1)
            except Exception:
                return []
    ...
    def _parse_agent_why_from_line(line: str) -> List[str]:
        """
        AGENT lines log why=a|b|c (pipe-separated).
        """
        mw = WHY_KV_RE.search(line)
        if not mw:
            return []
        raw = (mw.group("why") or "").strip()
        ...
        try:
            for line in fp.read_text(encoding="utf-8", errors="ignore").splitlines():
                ...
                if m_kind:
                    ...
                    # Fallback: support XAU agent execution confirmations without changing KIND_RE
                    if "[AGENT]" in line:
                        ma = AGENT_RE.search(line)
                        if ma:
                            kind = "AGENT"
                            sym = ma.group("sym")
                ...
                # -------------------------------------------
                # Extract policy/regime from key-value tokens
                # -------------------------------------------
                ...
                # Existing regime regex might match "regime=..."
                mr = REGIME_RE.search(line)
                if mr:
                    regime = mr.group("regime")
                else:
                    # XAU uses atr_regime=...
                    mar = ATR_REGIME_KV_RE.search(line)
                    if mar:
                        regime = mar.group("atr")
                ...
                if kind == "DECIDE":
                    ...
                    reasons = _parse_preview_reasons_from_line(line)
                    ...
                    # Attach most recent decide/preview
                    reasons: List[str] = []
                    pol = None
                    reg = None

                    # 1️⃣ Prefer DECIDE (authoritative)
                    if sym in last_decide:
                        ts0, why0, pol0, reg0 = last_decide[sym]
                        if abs((ts - ts0).total_seconds()) <= 30:
                            if why0:
                                reasons = why0
                            pol = pol0 or pol
                            reg = reg0 or reg

                    # 2️⃣ Fallback to PREVIEW if DECIDE empty
                    if sym in last_preview:
                        ts1, why1, pol1, reg1 = last_preview[sym]
                        if abs((ts - ts1).total_seconds()) <= 30:
                            if not reasons and why1:
                                reasons = why1
                            pol = pol or pol1
                            reg = reg or reg1
                    ...
                    # Treat AGENT "ok" lines as execution events (XAU compatibility)
                    # Only count when it is an "ok" execution confirmation
                    if " ok " not in line and " ok(" not in line:
                        continue

                    reasons = _parse_agent_why_from_line(line)

                    # If we didn't get reasons from why=, fallback to recent decide/preview within 10s
                    pol = policy
                    reg = regime
...
def enrich_loss_with_context(
    ...
    """
    Step 3 — Upstream Join
    ---------------------
    Enrich loss attribution with policy / atr_regime / session
    from log_daily_by_symbol_<tag>.csv

    Join keys (STRICT):
      date + symbol + tag
    """

    if loss_df.empty or not context_csv.exists():
        return
    ...
    ctx["date"] = pd.to_datetime(ctx["date"], errors="coerce").dt.date.astype(str)
    ctx["symbol"] = ctx["symbol"].fillna("UNKNOWN").astype(str)
    
    # Ensure tag column exists (older log_daily_by_symbol CSVs don't have it)
    
    if "tag" not in ctx.columns:
        ctx["tag"] = tag
    else:
        ctx["tag"] = ctx["tag"].fillna(tag)


    # Columns we would LIKE to have (but may not exist for all assets)
    wanted_cols = [
        "date",
        "symbol",
        "tag",
    ...
    # Now select safely
    ctx = ctx[wanted_cols].drop_duplicates()
    ...
    out_fp = outdir / f"analysis_loss_by_reason_by_symbol_{tag}_enriched.csv"
...
def main() -> int:
    ...
    if not loss_by_reason.empty:
        ...
        # Step 3 — Upstream Join (context enrichment)
        context_csv = outdir / f"log_daily_by_symbol_{args.tag}.csv"
        enrich_loss_with_context(
