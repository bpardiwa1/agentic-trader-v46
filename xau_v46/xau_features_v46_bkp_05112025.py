...
def compute_features(symbol: str) -> dict | None:
    ...
    try:
        ...
        log.info("[DEBUG] %s TF=%s EMA_FAST=%.2f EMA_SLOW=%.2f RSI=%.2f ATR%%=%.4f | RAW=%.2f ADJ=%.2f WHY=%s",
                 symbol, timeframe, ema_fast_v, ema_slow_v, rsi_v, atr_pct, features["raw_conf"], features["adj_conf"], why)
        return features
